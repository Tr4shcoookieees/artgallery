...
@section('content')
    <section style="background-image: url('{{asset('assets/banner.jpg')}}')" class="flex h-auto w-full flex-nowrap items-center justify-center bg-cover bg-fixed bg-bottom bg-blend-color-burn sm:h-96">
        <div class="flex h-full w-full flex-col items-center justify-center text-white font-light gap-y-2 bg-white bg-opacity-10 px-6 py-6 text-center backdrop-blur-sm backdrop-filter sm:px-16 sm:py-10 md:h-auto lg:w-auto">
            <h1 class="text-2xl uppercase sm:text-3xl">Купить искусство стало намного проще</h1>
            <p>{{config('app.name')}} - именно то место, которое вы так долго искали!</p>
            <p>Бесплатная доставка, справедливые цены, более миллиона работ со всего мира на продажу!</p>
            <x-link-primary href="/er" class="mt-2 hover:bg-opacity-10">Подробнее</x-link-primary>
        </div>
    </section>
    <main class="md:px-14">
        ...
        <section class="mt-8 flex flex-col">
            ...
            <div class="mt-4 grid sm:grid-cols-2 sm:grid-rows-2 lg:grid-cols-4 lg:grid-rows-1 gap-4">
                <figure class="relative">
                    <x-artwork-card-homepage/>
                </figure>
                <figure class="relative">

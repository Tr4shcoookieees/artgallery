...
@section('content')
    <div class="rounded bg-white p-8 ring-2 min-w-[384px] space-y-4 ring-primary">
        ...
        <form class="flex flex-col gap-y-3" method="post" action="{{ route('login') }}">
            @csrf
            ...
            <div class="mt-4 flex flex-col items-center justify-center">
                ...
                @endif
                <x-primary-button :value="__('Log in')"/>
                @if(Route::has('signup'))
                    <p class="mt-2 text-gray-400">{{__('Do not have an account?')}} <a class="underline" href="{{ route('signup') }}">{{__('Sign Up')}}</a></p>
                @endif
            </div>
        </form>
    </div>
